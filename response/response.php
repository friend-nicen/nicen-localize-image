	...
	public function response() {
		...
		try {
			...
			if ( isset( $_GET['nicen_make_clear_log'] ) ) {
				$this->auth(); //权限验证
				update_option( "nicen_plugin_error_log", "" ); //清空日志
				exit( json_encode( [
					'code'   => 1,
					'result' => "清除成功！"
				] ) );
			...
			if ( isset( $_GET['nicen_make_batch'] ) ) {
				...
				if ( ! empty( $json['category'] ) ) {
					$condition[] = '`ID` in (select DISTINCT `object_id` from `' . $table_prefix . 'term_relationships` where `term_taxonomy_id` in (' . join( ',', $json['category'] ) . '))';
				}


				$sql = 'select `ID` from `' . $table_prefix . 'posts` where ' . join( ' and ', $condition ) . ' order by `post_date`';
